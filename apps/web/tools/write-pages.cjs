...
function write(rel, content) {
  const fixed = content.replace(/<<DNODE>>/g, "<div").replace(/<\/DNODE>>/g, "</motion>").replace(/<\/DNODE>>/g, "</div>");
...
export function JobSetup() {
  ...
  return (
    <JobWorkspace>
      ...
      <<DNODE>> className="grid gap-5 lg:grid-cols-2">
        <section className="card card-pad">
          ...
          <<DNODE>> className="mt-4 flex gap-2">
        ...
        <section className="card card-pad">
          ...
          {!parsed ? <p className="mt-4 text-sm text-ink-muted">Parse a JD to preview requirements.</p> : (
            <<DNODE>> className="mt-4 space-y-4 text-sm">
              ...
              <<DNODE>> className="flex flex-wrap gap-2">
      ...
      {parsed && id ? (
        <section className="card card-pad mt-5">
          ...
          <<DNODE>> className="mt-3 flex flex-wrap gap-2">
            {PRD_SOURCES.map((s) => (
